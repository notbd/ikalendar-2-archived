...
public extension View {
    ...
    func listSeparatorStyle(_ style: ListSeparatorStyle, color: UIColor? = nil, inset: EdgeInsets? = nil) -> some View {
        modifier(ListSeparatorModifier(style: style, color: color, inset: inset, hideOnEmptyRows: false))
    }
    ...
    func listSeparatorStyle(_ style: ListSeparatorStyle, color: UIColor? = nil, inset: EdgeInsets? = nil, hideOnEmptyRows: Bool) -> some View {
...
private enum ListConstants {
    ...
    static let customDividerTag = 8_675_309
}
...
private struct ListSeparatorModifier: ViewModifier {
    ...
    private func adjust(divider: UIView) {
        divider.backgroundColor = color ?? .lightGray
...
private struct DividerLineSeekerView: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: InjectView, context _: Context) {
...
class DividerHandlingView: UIView {
    ...
    func updateDividers() {
        guard let hostingView = getHostingView(view: self) else { return }
        handleDividerLineSubviews(of: hostingView)
    }

    func getHostingView(view _: UIView) -> UIView? {
